...
const BUILT_IN_GEOJSON_DATASETS = {
  centralAsia: {
    name: "Central Asia polygon sample",
    path: "./samples/central-asia.geojson",
    defaults: {
      field: "water_stress_percent",
      title: "Central Asia Water Stress",
      year: "Sample",
      description: "GeoJSON polygon rasterized to the Aral grid contour system",
      source: "samples/central-asia.geojson"
    }
  }
};

const state = {
  datasetId: "centralAsia",
  records: [],
  analysis: null,
  field: "water_stress_percent",
  unit: "%",
...
function init() {
  for (const dataset of DATASETS) addOption(els.datasetSelect, dataset.id, dataset.name);
  for (const [id, dataset] of Object.entries(BUILT_IN_GEOJSON_DATASETS)) addOption(els.datasetSelect, id, dataset.name);
...
function bindEvents() {
  els.datasetSelect.addEventListener("change", () => {
    ...
    if (BUILT_IN_GEOJSON_DATASETS[state.datasetId]) return loadBuiltInGeojson(state.datasetId);
    const dataset = DATASETS.find((item) => item.id === state.datasetId);
    if (!dataset) return render();
  ...
  for (const [key, input] of [["title", els.titleInput], ["year", els.yearInput], ["description", els.descriptionInput], ["source", els.sourceInput]]) {
...
function updateAnalysis() {
  ...
  if (!state.analysis.numericFields.includes(state.field)) state.field = state.analysis.numericFields[0] || "";
  state.unit = detectUnit(state.field, state.analysis.metadata);
}
...
function syncControls() {
  ...
  for (const field of state.analysis.numericFields) addOption(els.fieldSelect, field, field);
  ...
  for (const button of els.presetButtons.querySelectorAll("button")) button.classList.toggle("active", button.dataset.preset === state.preset);
  for (const button of els.viewButtons.querySelectorAll("button")) button.classList.toggle("active", button.dataset.view === state.view);
}
...
function render() {
  ...
  const values = state.records.map((record) => Number(record[state.field])).filter(Number.isFinite);
  if (!state.field || !values.length) return diagnostic("Numeric field required");
  if (!state.analysis.spatial.detected) return diagnostic(state.analysis.spatial.message);
  const svg = renderGenericSvg({
    ...
    labels: { title: state.title, year: state.year, description: state.description, source: state.source, geographic: state.labels },
    view: state.view
  });
...
function parseGeoJson(geojson) {
  return geojson.features.map((feature, index) => ({ id: feature.id ?? index, ...feature.properties, geometry: feature.geometry }));
}

function parseSvg(text) {
  const doc = new DOMParser().parseFromString(text, "image/svg+xml");
  return [...doc.querySelectorAll("circle")].map((node, index) => ({ id: index, x: Number(node.getAttribute("cx")), y: Number(node.getAttribute("cy")), value: Number(node.getAttribute("r")) || 1 }));
}
...
function exportRaster(format) {
  const svg = els.preview.querySelector("svg");
  if (!svg) return;
  const viewBox = svg.getAttribute("viewBox")?.split(/\s+/).map(Number) || [0, 0, 480, 180];
  const scale = 4;
