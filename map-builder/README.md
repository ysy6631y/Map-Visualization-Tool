...
For generic GeoJSON polygon data, the Builder follows the Aral renderer pattern: geometry is sampled into the drawing grid, map boundaries are generated as cell-edge contours, and class dots are drawn from the same occupied grid cells.

## Existing System Relationship

- Visual token source: the original Aral Sea SVG outputs and `scripts/build-aral-sea-change.py`
- Runtime sample: `samples/central-asia.geojson`
...
Portrait and Landscape keep the original page sizes and near-full-page map fields. Expanding the visible map field beyond those measured margins would change grid cell counts, dot sampling, contour shape, and legend/source spacing, so the Builder treats the existing margins as part of the output design system.
